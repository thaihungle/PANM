...
def cuda(x, grad=False, gpu_id=-1):
  if gpu_id == -1:
    ...
    return var(x.pin_memory(), requires_grad=grad).cuda(gpu_id)
...
def cudavec(x, grad=False, gpu_id=-1):
  if gpu_id == -1:
    ...
    return var(T.from_numpy(x).pin_memory(), requires_grad=grad).cuda(gpu_id)
...
def cudalong(x, grad=False, gpu_id=-1):
  if gpu_id == -1:
    ...
    return var(T.from_numpy(x.astype(np.long)).pin_memory(), requires_grad=grad).cuda(gpu_id)
